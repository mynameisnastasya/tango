...
export default function AdminPage(){
  ...
  async function login(adminKey=key){
    ...
    try{
      ...
      const refRows=r.ok?await r.json():[{code:'KCu4ZY',recruiter:'ABHI AGANCY Manager',market:'International',onboarding_bonus:'Current onboarding bonus confirmed by manager',active:true}]
  ...
  if(!logged)return <main className="admin-login">
    ...
    <div className="login-card">
      <div className="brand"><span className="brand-mark">A</span><span>ABHI <b>AGANCY</b></span></div>
      <KeyRound size={30}/>
  ...
  return <main className="admin-root">
    ...
    <section className="admin-main">
      <header className="admin-topbar">
        <div><small>ABHI AGANCY</small><h1>{tab==='applications'?'Applicants':tab==='referrals'?'Referral Performance':'Agency Settings'}</h1></div>
